...
interface SilentCallbackMessage {
  type: 'iriai_silent_auth_callback';
  code?: string;
  error?: string;
  errorDescription?: string;
  state?: string;
 ...
 */
function isSilentCallbackMessage(data: unknown): data is SilentCallbackMessage {
  return (
    typeof data === 'object' &&
    data !== null &&
    (data as SilentCallbackMessage).type === 'iriai_silent_auth_callback'
  );
}
 ...
 */
function encodeState(originalState: string, origin: string): string {
 ...
 */
export function isSilentAuthLikelyBlocked(): boolean {
  ...
  if (isSafari || isBrave) {
    console.warn(
      '[SilentAuth] Browser may block third-party cookies. Silent auth might fail.'
    );
    return true;
  }
