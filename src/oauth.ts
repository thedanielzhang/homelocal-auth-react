...
export function createOAuthClient(config: OAuthConfig): OAuthClient {
   ...
   */
  function resolveAuthServiceUrl(options?: { authServiceUrl?: string }): string {
    const url = options?.authServiceUrl || defaultAuthServiceUrl;
    if (!url) {
      throw new Error(
        'authServiceUrl is required. Provide it in OAuthConfig or pass it in options.'
      );
    }
    return url;
  }
